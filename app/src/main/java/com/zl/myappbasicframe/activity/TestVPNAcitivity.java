...
public class TestVPNAcitivity extends Activity implements OnClickListener {
    private static final String TAG = "TestVPNAcitivity";
    ...
    // VPN服务器地址、端口号、用户名、密码
    private final String VPN_SERVER = "211.160.21.193 ";
    private final int VPN_PORT = 8080;
    private final String VPN_USERNAME = "xiazheng";
    private final String VPN_PASSWORD = "zhiren@123";
            ...
            switch (msg.what) {
                case VPN_MSG_STATUS_UPDATE: // VPN库发送消息处理
                {
                    if (null != bundle) {
                        ...
                        if (vpnStatus.equalsIgnoreCase("6")) {
                            ...
                            Toast.makeText(TestVPNAcitivity.this, "VPN库消息通知：VPN隧道建立成功",
                                    Toast.LENGTH_SHORT).show();
                        }

                        if (vpnStatus.equalsIgnoreCase("200")) {
                            Toast.makeText(TestVPNAcitivity.this, "VPN库消息通知：VPN隧道超时",
                                    Toast.LENGTH_SHORT).show();
                        }
                        ...
                        if (!vpnErr.equalsIgnoreCase("0")) {
                            ...
                            if (vpnErr.equalsIgnoreCase("10")) {
                                ...
                                // VPN隧道建立出错
                                Toast.makeText(TestVPNAcitivity.this,
                ...
                case QUERY_VPN_MSG_STATUS_UPDATE: // 查询线程消息处理
                {
                    if (null != bundle) {
                        ...
                        if (vpnStatus.equalsIgnoreCase("6")) {
                            // VPN隧道建立成功
                            Toast.makeText(TestVPNAcitivity.this, "查询线程通知：VPN隧道建立成功",
                                    Toast.LENGTH_SHORT).show();
                        }
    ...
    private void doGetVPNStatus() {
        ...
        Toast.makeText(TestVPNAcitivity.this, "当前VPN状态为：" + sVPNStatus,
                Toast.LENGTH_SHORT).show();

        ArrayList<HashMap<String, String>> reslist = vpnlib.getResList();
        Log.i("test", "port1 :" + vpnlib.getResLocalPort("211.160.21.193 ", 8080));
        Log.i("test", "port2 :" + vpnlib.getResLocalPort("211.160.21.193 ", 8080));
